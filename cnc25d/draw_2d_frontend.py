...
def get_center(CX=0.0, CY=0.0, C=()):
  ...
  if(len(C)>0):
    if((CX!=0)or(CY!=0)):
      six.print_(("ERR112: Error, CX {:0.3f} or CY {:0.3d} have been set simultaneously with C".format(CX, CY)))
      sys.exit(2)
    if(len(C)!=2):
      six.print_(("ERR115: len(C) {:d} is not 2".format(len(C))))
      sys.exit(2)
    OCX = C[0]
    OCY = C[1]
  r = (OCX, OCY)
...
def c_xy(d, a, CX=0.0, CY=0.0, C=()):
  ...
  # check distance
  if(d<0):
    six.print_(("ERR104: Error, the polar coordinate distance d {:0.3f} is negative".format(d)))
    sys.exit(2)
  # origin-center
...
class Arc_Line_Outline:
  ...
  def add_StartPoint(self, CX=0.0, CY=0.0, C=(), rbr=0.0):
    ...
    """
    if(self.ol!=[]):
      six.print_(("ERR152: Error, StartPoint can not be added to the outline {:s} with {:d} segments".format(self.outline_id, len(self.ol))))
      sys.exit(2)
  ...
  def add_LineTo(self, CX=0.0, CY=0.0, C=(), rbr=0.0):
    ...
    """
    if(len(self.ol)<1):
      six.print_(("ERR163: Error, line-segment can not be added to the outline {:s} with {:d} segments. Add StartPoint first.".format(self.outline_id, len(self.ol))))
      sys.exit(2)
    (X, Y) = get_center(CX, CY, C)
    self.ol.append((X, Y, rbr))
  ...
  def add_ArcThrTo(self, CX1=0.0, CY1=0.0, CX2=0.0, CY2=0.0, C1=(), C2=(), rbr=0.0):
    ...
    """
    if(len(self.ol)<1):
      six.print_(("ERR174: Error, arc-segment can not be added to the outline {:s} with {:d} segments. Add StartPoint first.".format(self.outline_id, len(self.ol))))
      sys.exit(2)
  ...
  def close_with_Line(self):
    ...
    """
    if(len(self.ol)<2):
      six.print_(("ERR157: Error, line-segment can not be added to close the outline {:s} with {:d} segments. Add at least one segment first.".format(self.outline_id, len(self.ol))))
      sys.exit(2)
    X = self.ol[0][0]
    Y = self.ol[0][1]
  ...
  def close_with_ArcThr(self, CX1=0.0, CY1=0.0, C1=()):
    ...
    """
    if(len(self.ol)<2):
      six.print_(("ERR168: Error, arc-segment can not be added to close the outline {:s} with {:d} segments. Add at least one segment first.".format(self.outline_id, len(self.ol))))
      sys.exit(2)
    (X1, Y1) = get_center(CX1, CY1, C1)
    X2 = self.ol[0][0]
    Y2 = self.ol[0][1]
  ...
  def close_insurance(self):
    ...
    if((abs(self.ol[0][0]-self.ol[-1][-3])>radian_epsilon)or(abs(self.ol[0][1]-self.ol[-1][-2])>radian_epsilon)):
      six.print_(("ERR206: Error, outline {:s} should be closed but is not closed: first point ({:0.3f}, {:0.3f}), last point ({:0.3f}, {:0.3f})".format(self.outline_id, self.ol[0][0], self.ol[0][1], self.ol[-1][-3], self.ol[-1][-2])))
      sys.exit(2)
  ...
  def check(self, figure_id=""):
    ...
    #
    if(len(self.ol)<2):
      six.print_(("ERR179: Error, figure {:s}, outline {:s} with {:d} segments is too short. Add at leat two segment first.".format(figure_id, self.outline_id, len(self.ol))))
      sys.exit(2)
    if(len(self.ol[0])!=3):
      six.print_(("ERR222: Error, figure {:s}, outline {:s}, the StartPoint_length {:d} is not 3".format(figure_id, self.outline_id, len(self.ol[0]))))
      sys.exit(2)
    for i in range(len(self.ol)):
      if((len(self.ol[i])!=3)and(len(self.ol[i])!=5)):
        six.print_(("ERR226: Error, figure {:s}, outline {:s}, segment {:d}, length {:d} is not 3 or 5".format(figure_id, self.outline_id, i, len(self.ol[i]))))
        sys.exit(2)
    if((self.ol[0][0]!=self.ol[-1][-3])or(self.ol[0][1]!=self.ol[-1][-2])):
      six.print_(("ERR182: Error, figure {:s}, outline {:s} is not closed: first point ({:0.3f}, {:0.3f}), last point ({:0.3f}, {:0.3f})".format(figure_id, self.outline_id, self.ol[0][0], self.ol[0][1], self.ol[-1][-3], self.ol[-1][-2])))
      sys.exit(2)
    if(self.ol[-1][-1]!=0):
      six.print_(("ERR185: Error, figure {:s}, outline {:s}, last point router_bit_radius {:0.3f} is not 0.0".format(figure_id, self.outline_id, self.ol[-1][-1])))
      sys.exit(2)
    ...
    for i in range(len(self.ol)-1):
      ...
      if(len(self.ol[i+1])==3):
        ...
        six.print_(("ERR244: Error, figure_id {:s}, outline {:s}, segment {:d} unexpected length {:d}".format(figure_id, self.outline_id, i+1, len(self.ol[i+1]))))
        sys.exit(2)
      if(pt1_distance<radian_epsilon):
        six.print_(("ERR258: Error, figure_id {:s}, outline {:s}, segment {:d} pt1_distance {:0.3f} too small".format(figure_id, self.outline_id, i+1, pt1_distance)))
        sys.exit(2)
      if((pt2_distance<radian_epsilon) and not(segment_line_narc)):
        six.print_(("ERR261: Error, figure_id {:s}, outline {:s}, segment {:d} pt2_distance {:0.3f} too small".format(figure_id, self.outline_id, i+1, pt2_distance)))
        sys.exit(2)
      # string_length
...
""".format(ol_stat['x_min'], ol_stat['x_max'], ol_stat['y_min'], ol_stat['y_max'], ol_stat['x_max']-ol_stat['x_min'], ol_stat['y_max']-ol_stat['y_min'])
  ...
  def convert_from_old_format(self, ai_outline):
    ...
    """
    if(len(ai_outline)<3):
      six.print_(("ERR378: Error, outline {:s}, convert_from_old_format failed because import list {:d} is too short".format(self.outline_id, len(ai_outline))))
      sys.exit(2)
    first_segment = ai_outline[0]
    if(len(first_segment)!=3):
      six.print_(("ERR381: Error, outline {:s}, convert_from_old_format failed because import first point length {:d} is not 3".format(self.outline_id, len(first_segment))))
      sys.exit(2)
    ...
    for i in range(len(ai_outline)-2):
      ...
      if(len(segment)==3):
        ...
        six.print_(("ERR390: Error, outline {:s}, convert_from_old_format failed because segment length {:d} is not 3 or 5".format(self.outline_id, len(segment))))
        sys.exit(2)
    last_segment = ai_outline[-1]
    if(last_segment[-1]!=0):
      six.print_(("ERR395: Error, outline {:s}, convert_from_old_format failed because import last_point rbr {:0.3f} is not 0.0".format(self.outline_id, last_segment[-1])))
      sys.exit(2)
    if((last_segment[-3]!=first_segment[0])or(last_segment[-2]!=first_segment[1])):
      six.print_(("ERR399: Error, outline {:s}, convert_from_old_format failed because import is not closed x {:0.3f} {:0.3f} y {:0.3f} {:0.3f}".format(self.outline_id, first_segment[0], last_segment[-3], first_segment[1], last_segment[-2])))
      sys.exit(2)
    if(len(last_segment)==3):
      ...
      six.print_(("ERR406: Error, outline {:s}, convert_from_old_format failed because last_segment length {:d} is not 3 or 5".format(self.outline_id, len(last_segment))))
      sys.exit(2)
...
class Circle_Outline:
  ...
  def __init__(self, outline_id, R, CX=0.0, CY=0.0, C=()):
    ...
    # check arguments
    if(R<0):
      six.print_(("ERR151: Error, the radius R {:0.3f} is negative".format(R)))
      sys.exit(2)
    (X, Y) = get_center(CX, CY, C)
    # save values
    self.outline_id = outline_id
  ...
  def check(self, figure_id=""):
    ...
    if(self.radius<radian_epsilon):
      six.print_(("ERR361: Error, figure {:s}, circle {:s} with radius {:.3f} too small".format(figure_id, self.outline_id, self.radius)))
      sys.exit(2)
...
""".format(self.radius, 2*self.radius, self.x, self.y, ol_stat['circle_circumference'], ol_stat['x_min'], ol_stat['x_max'], ol_stat['y_min'], ol_stat['y_max'])
  ...
  def convert_from_old_format(self, ai_outline):
    ...
    """
    if(len(ai_outline)!=3):
      six.print_(("ERR483: Error, outline {:s}, convert_from_old_format failed because import list {:d} is not 3".format(self.outline_id, len(ai_outline))))
      sys.exit(2)
    self.x = ai_outline[0]
    self.y = ai_outline[1]
...
class Figure:
  ...
  def _add_outline(self, outline, hole_check=False):
    ...
    if(hole_check):
      if((self.stat['x_min']>ol_stat['x_min'])or(self.stat['x_max']<ol_stat['x_max'])or(self.stat['y_min']>ol_stat['y_min'])or(self.stat['y_max']<ol_stat['y_max'])):
        six.print_(("ERR417: Error, figure {:s}, hole-ouline {:s} outside the boundary: x_min {:0.3f} {:0.3f}, x_max {:0.3f} {:0.3f}, y_min {:0.3f} {:0.3f}, y_max {:0.3f} {:0.3f}".format(self.figure_id, outline.outline_id, self.stat['x_min'], ol_stat['x_min'], self.stat['x_max'], ol_stat['x_max'], self.stat['y_min'], ol_stat['y_min'], self.stat['y_max'], ol_stat['y_max'])))
        sys.exit(2)
    ...
    if(isinstance(outline, Circle_Outline)):
      ...
      six.print_(("ERR429: Error, figure {:d}, outline {:s} is an invalid object".format(self.figure_id, outline.outline_id)))
      sys.exit(2)
  ...
  def add_external_outline(self, outline):
    ...
    if(len(self.outlines)!=0):
      six.print_(("ERR457: Error, figure {:s}, external_outline {:s} not added at the first position {:d}".format(self.figure_id, outline.outline_id, len(self.outlines))))
      sys.exit(2)
  ...
  def add_hole_outline(self, outline):
    ...
    if(len(self.outlines)==0): # check if there is already an external_outline
      six.print_(("ERR465: Error, figure {:s}, hole_outline {:s} added before external_outline".format(self.figure_id, outline.outline_id)))
      sys.exit(2)
    if(not self.extrudable): # check no undefine_outline have been added
      six.print_(("ERR468: Error, figure {:s}, hole_outline {:s} added to a not extrudable figure {:d}".format(self.figure_id, outline.outline_id, len(self.outlines))))
      sys.exit(2)
...
""".format(self.stat['x_min'], self.stat['x_max'], self.stat['y_min'], self.stat['y_max'], self.stat['x_max']-self.stat['x_min'], self.stat['y_max']-self.stat['y_min'])
  ...
  def get_outline_index(self, ai_index):
    ...
    if(abs(ai_index)>=len(self.outlines)):
      six.print_(("ERR544: Error, figure {:s}, outline index {:d} is out of the range {:s}".format(self.figure_id, ai_index, len(self.outlines))))
      sys.exit(2)
  ...
  def get_outline_id(self, ai_outline_id):
    ...
    for i in range(len(self.outlines)):
      if(self.outlines[i].outline_id==ai_outline_id):
        idx = i
    if(idx==-1):
      six.print_(("ERR558: Error, figure {:s}, outline_id {:s} does not exist".format(self.figure_id, ai_outline_id)))
      sys.exit(2)
  ...
  def convert_from_old_format(self, ai_figure):
    ...
    """
    if(len(ai_figure)<1):
      six.print_(("ERR701: Error, figure {:s}, convert_from_old_format failed because import list {:d} is 0".format(self.figure_id, len(ai_figure))))
      sys.exit(2)
...
class Figure_Collection:
  ...
  def add_figure(self, ai_figure):
    ...
    if(not isinstance(ai_figure, Figure)):
      six.print_(("ERR525: Error, figure_collection {:s}, add unexpected object {:s}".format(self.collection_id, ai_figure.figure_id)))
      sys.exit(2)
...
""".format(self.stat['dx_min'], self.stat['dx_max'], self.stat['dy_min'], self.stat['dy_max'])
  ...
  def get_figure_index(self, ai_index):
    ...
    if(abs(ai_index)>=len(self.figures)):
      six.print_(("ERR583: Error, figure_collection {:s}, figure index {:d} is out of the range {:s}".format(self.collection_id, ai_index, len(self.figures))))
      sys.exit(2)
  ...
  def get_figure_id(self, ai_figure_id):
    ...
    for i in range(len(self.figures)):
      if(self.figures[i].figure_id==ai_figure_id):
        idx = i
    if(idx==-1):
      six.print_(("ERR597: Error, figure_collection {:s}, figure_id {:s} does not exist".format(self.collection_id, ai_figure_id)))
      sys.exit(2)
...
def test_c_xy():
  ...
  six.print_(("c_xy 5.0, 1.0, 0, 0 : x {:0.3f} , y {:0.3f}".format(x,y)))
  (x,y) = c_xy(3.0, -1.0, 2.0, -1.0)
  six.print_(("c_xy 3.0, -1.0, 2, -1.0 : x {:0.3f} , y {:0.3f}".format(x,y)))
  (x,y) = c_xy(5.0, 1.0, C=(-2,-3))
  six.print_(("c_xy 5.0, 1.0, C=(-2,-3) : x {:0.3f} , y {:0.3f}".format(x,y)))
...
def test_draw_2d_1():
  ...
  tfc.add_figure(fig3)
  six.print_((tfc.stat_info()))
  six.print_((tfc.get_figure_id('first_figure').stat_info("test_figure_collection")))
  six.print_((tfc.get_figure_id('first_figure').get_outline_id('inner_shape').stat_info("test_figure_collection.first_figure")))
  six.print_((tfc.get_figure_id('first_figure').get_outline_id('inner_circle').stat_info("test_figure_collection.first_figure")))
  ...
  six.print_((tfc2.stat_info()))
  six.print_((tfc2.get_figure_id('first_figure').stat_info("test_figure_collection_reload")))
