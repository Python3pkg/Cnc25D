...
class bare_design:
  ...
  def apply_constraint(self, constraint):
    ...
    for k in constraint.keys():
      if(not k in rc.keys()):
        six.print_(("ERR177: Error, contraint {:k} is not part of the design".format(k)))
        sys.exit(2)
      if(c[k] != constraint[k]):
  ...
  def apply_external_constraint(self, constraint):
    ...
    """
    key_list = self.reference_constraint & constraint
  ...
  def get_A_figure(self, figure_id=""):
    ...
    if(not figure_id in self.A_figures.keys()):
      six.print_(("ERR156: Error, figure_id {:s} is not in the figure list [{:s}]".format(figure_id, ' '.join(self.A_figures.keys()))))
      sys.exit(2)
    ...
    if(figs != None):
      ...
      for f in r_list:
        if(not f in self.A_figures.keys()):
          six.print_(("ERR304: Error, f {:s} is not an existing 2d-figures {:s}".format(f, ' '.join(self.A_figures.keys()))))
          sys.exit(2)
    return(r_list)
  ...
  def get_fc_obj_3dconf(self, assembly_name=""):
    ...
    if(not assembly_name in self.assembly_configurations.keys()):
      six.print_(("ERR187: Error, the assembly_name {:s} is not in the possible 3d-assembly-configuration".format(assembly_name)))
      sys.exit(2)
  ...
  def get_fc_obj_function(self, function_id=""):
    ...
    if(not function_id in self.freecad_function_pts.keys()):
      six.print_(("ERR370: Error, the function_id {:s} is not in the possible 3d-freecad_function_pts".format(function_id)))
      sys.exit(2)
  ...
  def write_info_txt(self, output_file_basename):
    ...
    six.print_(("Generate the text info file {:s}".format(info_txt_filename)))
  ...
  def get_write_2d_figure_list(self):
    ...
    if(figs != None):
      ...
      for f in r_list:
        if(not f in self.A_figures.keys()):
          six.print_(("ERR291: Error, f {:s} is not an existing 2d-figures {:s}".format(f, ' '.join(self.A_figures.keys()))))
          sys.exit(2)
    return(r_list)
  ...
  def get_write_3d_figure_list(self):
    ...
    if(figs != None):
      ...
      for f in r_list:
        if(not f in self.A_figures.keys()):
          six.print_(("ERR380: Error, f {:s} is not an existing 2d-figures {:s}".format(f, ' '.join(self.A_figures.keys()))))
          sys.exit(2)
    return(r_list)
    ...
    if(confs != None):
      ...
      for f in r_list:
        if(not f in self.assembly_configurations.keys()):
          six.print_(("ERR406: Error, f {:s} is not an existing 3d-assembly-configurations {:s}".format(f, ' '.join(self.assembly_configurations.keys()))))
          sys.exit(2)
    return(r_list)
  ...
  def write_assembly_brep(self, output_file_basename, ai_brep=True, ai_stl=False):
    ...
    for a in confs:
      six.print_(("write_assembly_brep: {:s}".format(a)))
    ...
    if(l != None):
      ...
      for f in r_list:
        if(not f in self.freecad_function_pts.keys()):
          six.print_(("ERR460: Error, f {:s} is not an existing 3d-freecad_function_pts {:s}".format(f, ' '.join(self.freecad_function_pts.keys()))))
          sys.exit(2)
    return(r_list)
  ...
  def run_simulation(self, sim_id=''):
    ...
    if(len(self.simulation_2d_pts)==0):
      six.print_(("ERR268: Error, no simulation function is provided. Can't run 2d-simulate {:s}".format(sim_id)))
      sys.exit(2)
    if(sim_id==''):
      sim_id = self.simulation_2d_pts.keys()[0]
    if(not sim_id in self.simulation_2d_pts.keys()):
      six.print_(("ERR382: Error, the simulation id {:s} does not exist in the list {:s}".format(sim_id, ' '.join(self.simulation_2d_pts.keys()))))
      sys.exit(2)
    six.print_(("SIMULATION: {:s} runs simulation: {:s}".format(self.design_name, sim_id)))
  ...
  def view_design_configuration(self):
    ...
    fig_ids = figs.keys()
    six.print_(("{:s} 2D-figures:".format(self.design_name)))
    if(len(fig_ids)==0):
      print("No 2D-figures")
    else:
      for i in range(len(fig_ids)):
        six.print_(("{:02d} : {:s}".format(i, fig_ids[i])))
    # 2D-simulations
    sim_ids = self.simulation_2d_pts.keys()
    six.print_(("{:s} 2D-simulations:".format(self.design_name)))
    if(len(sim_ids)==0):
      print("No 2D-simulation")
    else:
      for i in range(len(sim_ids)):
        six.print_(("{:02d} : {:s}".format(i, sim_ids[i])))
    ...
    if(self.f_3d_constructor==None):
      ...
      six.print_(("{:s} assembly_3dconf:".format(self.design_name)))
      for i in range(len(assembly_ids)):
        six.print_(("{:02d} : {:s}".format(i, assembly_ids[i])))
    ...
    if(self.f_3d_freecad_constructor==None):
      ...
      six.print_(("{:s} freecad_function_pts:".format(self.design_name)))
      for i in range(len(fc_ids)):
        six.print_(("{:02d} : {:s}".format(i, fc_ids[i])))
    # display_2d_figure_list
    six.print_(("{:s} display_2d_figure_list:".format(self.design_name)))
    figs = self.get_display_2d_figure_list()
    for i in range(len(figs)):
      six.print_(("{:02d} : {:s}".format(i, figs[i])))
    # default_simulation
    six.print_(("{:s} default_simulation: {:s}".format(self.design_name, self.default_simulation)))
    # file_2d_figure_file_list
    six.print_(("{:s} svg-dxf-file_2d_figure_list:".format(self.design_name)))
    figs = self.get_write_2d_figure_list()
    for i in range(len(figs)):
      six.print_(("{:02d} : {:s}".format(i, figs[i])))
    # file_3d_figure_file_list
    six.print_(("{:s} brep-file_3d_figure_list:".format(self.design_name)))
    figs = self.get_write_3d_figure_list()
    for i in range(len(figs)):
      six.print_(("{:02d} : {:s}".format(i, figs[i])))
    # file_3d_conf_file_list
    six.print_(("{:s} brep-file_3d_conf_list:".format(self.design_name)))
    confs = self.get_write_3d_conf_list()
    for i in range(len(confs)):
      six.print_(("{:02d} : {:s}".format(i, confs[i])))
    # file_3d_freecad_file_list
    six.print_(("{:s} brep-file_3d_freecad_list:".format(self.design_name)))
    fc_l = self.get_write_3d_freecad_list()
    for i in range(len(fc_l)):
      six.print_(("{:02d} : {:s}".format(i, fc_l[i])))
    # self-test-list
    test_ids = []
    for i in range(len(self.self_tests)):
      test_ids.append(self.self_tests[i][0])
    six.print_(("design_self-test_list: {:s}".format(', '.join(test_ids))))
    # return (not yet used)
    return(fig_ids)
  ...
  def apply_cli_with_output_options(self, cli_str=""):
    ...
    if(oo_args.sw_return_type!=''):
      if(self.f_return_type==None):
        six.print_(("ERR277: Error, no return_type function is provided. Can't apply return_type {:s}".format(oo_args.sw_return_type)))
        sys.exit(2)
  ...
  def run_self_test(self, test_id=''):
    ...
    if(test_id==''):
      ...
      if(not test_id in test_ids):
        six.print_(("ERR541: Error, test_id {:s} is not in the test-list {:s}".format(test_id, ', '.join(test_ids))))
        sys.exit(2)
    ...
    for i in range(len(test_list)):
      ...
      for j in range(len(test_ids)):
        if(test_ids[j]==test_list[i]):
          tn=j
      if(tn==-1):
        six.print_(("ERR562: the test {:s} has not been found".format(test_list[i])))
        sys.exit(2)
      else:
  ...
  def dump_constraint_file(self, snippet_filename):
    ...
    py_txt = ""
    for (k,v) in six.iteritems(constraint_dict):
      #py_txt += "\t{:30s}\t= {:s}\n".format(k, str(v))
      py_txt += "c[\'{:s}\']\t= {:s}\n".format(k, str(v))
    six.print_(("Generate the python snippet file {:s}".format(snippet_filename)))
...
def bare_design_test1():
  ...
  def cube_constraint_check(c):
    ...
    if(c['width']<c['length']*0.1):
      six.print_(("ERR129: Error, width {:0.3f} is too small compare to length {:0.3f}".format(c['width'], c['length'])))
      sys.exit(2)
    if(c['width']>c['length']*10.0):
      six.print_(("ERR132: Error, width {:0.3f} is too big compare to length {:0.3f}".format(c['width'], c['length'])))
      sys.exit(2)
    if(c['height']<c['length']*0.1):
      six.print_(("ERR135: Error, height {:0.3f} is too small compare to length {:0.3f}".format(c['height'], c['length'])))
      sys.exit(2)
    if(c['height']>c['length']*10.0):
      six.print_(("ERR138: Error, height {:0.3f} is too big compare to length {:0.3f}".format(c['height'], c['length'])))
      sys.exit(2)
    if(c['smooth_radius']>0.4*min(c['length'], c['width'])):
      six.print_(("ERR182: Error, smooth_radius {:0.3f} is too big compare to length {:0.3f} or width {:0.3f}".format(c['smooth_radius'], c['length'], c['width'])))
      sys.exit(2)
    return(c)
